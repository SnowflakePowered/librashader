...
extern_fn! {
    ...
    fn libra_gl_filter_chain_create(
        ...
        let context = glow::Context::from_loader_function_cstr(
            |proc_name| unsafe { loader(proc_name.as_ptr()) })
        unsafe {
            let chain = FilterChain::load_from_preset(context, *preset, options.as_ref())?;
...
extern_fn! {
    ...
    nopanic fn libra_gl_filter_chain_frame(
        ...
        let framebuffer = GLFramebuffer::new_from_raw(chain.get_context(),
            texture, fbo, out.format, Size::new(viewport.width, viewport.height), 1);
