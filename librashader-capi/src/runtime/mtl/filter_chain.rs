...
#[repr(transparent)]
pub struct PMTLCommandQueue(ProtocolObject<dyn MTLCommandQueue>);

#[repr(transparent)]
pub struct PMTLCommandBuffer(ProtocolObject<dyn MTLCommandBuffer>);

#[repr(transparent)]
pub struct PMTLTexture(ProtocolObject<dyn MTLTexture>);
...
extern_fn! {
    ...
    fn libra_mtl_filter_chain_create_deferred(
        preset: *mut libra_shader_preset_t,
        queue: *const PMTLCommandQueue,
        command_buffer: *const PMTLCommandBuffer,
        options: *const MaybeUninit<filter_chain_mtl_opt_t>,
        out: *mut MaybeUninit<libra_mtl_filter_chain_t>
    ) |queue, command_buffer| {
        assert_non_null!(preset);
        assert_some_ptr!(queue);
         assert_some_ptr!(command_buffer);
        let preset = unsafe {
            let preset_ptr = &mut *preset;
            let preset = preset_ptr.take();
            Box::from_raw(preset.unwrap().as_ptr())
...
extern_fn! {
    ...
    nopanic fn libra_mtl_filter_chain_frame(
        ...
        command_buffer: *const PMTLCommandBuffer,
        frame_count: usize,
        image: *const PMTLTexture,
        viewport: libra_viewport_t,
        output: *const PMTLTexture,
        mvp: *const f32,
        opt: *const MaybeUninit<frame_mtl_opt_t>
    ) |command_buffer, image, output|; mut |chain|  {
        assert_some_ptr!(mut chain);
        assert_some_ptr!(command_buffer);
        assert_some_ptr!(image);
        assert_some_ptr!(output);
